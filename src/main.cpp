...
void printUsage(const char* programName) {
    std::cout << "CUDA Video Cleaner - Removes background noise and cleans video" << std::endl;
    ...
    std::cout << "  --force-cpu                 : Force CPU-based video processing even if CUDA is available" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    while (argIdx < argc) {
        if (strcmp(argv[argIdx], "--low-cutoff") == 0 && argIdx + 1 < argc) {
            ...
            argIdx += 2;
        } else if (strcmp(argv[argIdx], "--force-cpu") == 0) {
            forceCPU = true;
            argIdx++;
    ...
    std::string processingType = cudaAvailable && !forceCPU ? "CUDA-accelerated" : "CPU-based";
    ...
    try {
        ...
        VideoProcessor processor(lowCutoff, highCutoff, noiseReduction, videoDenoiseStrength, forceCPU);
