...
// CUDA kernel to convert RGB to YUV color space
...
// CUDA kernel to convert YUV back to RGB
...
void CUDAVideoDenoiser::initialize(int width, int height) {
    ...
    // Allocate CUDA memory for input/output frames and processing buffers
...
cv::Mat CUDAVideoDenoiser::denoise(const cv::Mat& inputFrame) {
    ...
    // Extract Y, U, V components for denoising
    ...
    // Strength is in the range [0, 100]
