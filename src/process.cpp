...
// Constructor
VideoProcessor::VideoProcessor(float lowCutoff, float highCutoff, float noiseReduction, float videoDenoiseStrength, bool forceCPU)
    : m_lowCutoff(lowCutoff), m_highCutoff(highCutoff), m_noiseReduction(noiseReduction), 
      m_videoDenoiseStrength(videoDenoiseStrength), m_forceCPU(forceCPU) {
    
    m_audioProcessor = nullptr;
    
    // Initialize video denoiser with appropriate implementation
    m_videoDenoiser = createVideoDenoiser(videoDenoiseStrength, forceCPU);
}
...
bool VideoProcessor::extractAudio(const std::string& videoPath, std::vector<float>& audioData, int& sampleRate, int& channels) {
    ...
    // Set up resampler to convert to float planar format
    ...
    // Set resampler options using the newer API with ch_layout struct directly
...
bool VideoProcessor::processAudio(std::vector<float>& audioData, int sampleRate, int channels) {
    ...
    // Create audio processor with the correct sample rate
...
cv::Mat VideoProcessor::denoiseFrame(const cv::Mat& frame) {
    ...
    // Denoise frame using whatever implementation we have (CPU or CUDA)
...
bool VideoProcessor::saveProcessedAudioToWav(const std::string& wavPath, const std::vector<float>& audioData, 
    ...
    // Write audio data (assuming float samples)
...
bool VideoProcessor::processVideoFrames(const std::string& inputPath, const std::string& outputPath, 
    ...
    // Set up output video
    std::string finalOutputPath = outputPath; // Save final path
    ...
    std::string denoiserType = isCudaAvailable() ? "CUDA" : "CPU";
    ...
    // Close videos
    inputVideo.release();
    outputVideo.release(); // This writes the video-only file to 'tempVideoFile'

    // --- Mux audio and video using FFmpeg ---
    std::string tempAudioPath = finalOutputPath + ".tmp_audio.wav"; // Use finalOutputPath for temp name base
    if (!saveProcessedAudioToWav(tempAudioPath, processedAudio, audioSampleRate, audioChannels)) {
        ...
        // Consider deleting tempVideoPath here if it was created by VideoWriter directly to outputPath
        return false; // Or indicate partial success if video-only is acceptable
    }
    ...
    if (ret == 0) {
        ...
        // Keep temp files for debugging if muxing fails
