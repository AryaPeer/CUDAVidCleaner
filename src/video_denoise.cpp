...
#ifdef WITH_CUDA
// Include CUDA headers only when compiling with CUDA support
#include <cuda_runtime.h>
// Include the header only if CUDA is enabled
...
bool isCudaAvailable() {
    ...
    // Return true if CUDA is available and at least one device is found
    return (error == cudaSuccess && deviceCount > 0);
#else
    // CUDA support was not compiled in
    return false;
#endif
}

// Factory function to create the appropriate denoiser
std::unique_ptr<VideoDenoiser> createVideoDenoiser(float strength, bool forceCPU) {
    if (!forceCPU && isCudaAvailable()) {
        // Include this section only when compiled with CUDA support
#ifdef WITH_CUDA
        try {
...
cv::Mat CPUVideoDenoiser::denoise(const cv::Mat& inputFrame) {
    ...
    // Use OpenCV's CPU-based denoising algorithms
    cv::Mat result;
